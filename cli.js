...
function main() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith('-')) {
      ...
      if (/^(-j|--jobs|--shard|--reporter|-t|--testNamePattern|--testTimeout|--retry|--bail|--maxWorkers|--minWorkers|--coverage-dir|--coverage-thresholds|--coverage-threshold|--coverage-reporter|--coverage-reporters|--coverage-include|--coverage-exclude)$/.test(a) && i + 1 < argv.length && !argv[i + 1].startsWith('-')) {
        flags.push(argv[++i]);
      }
    } else {
      files.push(a);
